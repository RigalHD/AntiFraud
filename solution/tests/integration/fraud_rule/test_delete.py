...
async def test_ok(
    ...
    (await api_client.delete_fraud_rule(fraud_rule.id)).expect_status(204)
...
async def test_no_auth(
    ...
    error_data = (await api_client.delete_fraud_rule(fraud_rule.id)).expect_status(401).err_unwrap()
...
async def test_forbidden(
    ...
    error_data = (await api_client.delete_fraud_rule(fraud_rule.id)).expect_status(403).err_unwrap()
...
async def test_not_found(
    ...
    error_data = (await api_client.delete_fraud_rule(uuid4())).expect_status(404).err_unwrap()
