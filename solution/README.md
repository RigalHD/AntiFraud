# Проект AntiFraud
- Проект делался в спешке, так что недочеты будут исправлены со временем
> [!WARNING]
> Существует проблема с запуском локальных тестов после запуска проекта (и наоборот).
> Для корректной работы требуется очистить весь контейнер командой `just clear`
